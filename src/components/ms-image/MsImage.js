...
export default class MsImage extends Component {
    ...
    async componentWillMount(){
        ...
        const newSrc = src+`?width=${width}&height=${height}`
        // await timeout(30000000)
        await Promise.all([axios(newSrc),timeout(delay)])
        this.setState({loaded: true, src: newSrc})
    }
    render() {
        const { as:Comp=Image, src:oldSrc, loading, width, children, style:oldStyle={}, height, block, ...otherProps } = this.props; 
        const { loaded, src } = this.state;
        const style={...oldStyle,
            display:(!!block)?'block':'inline-block'
        }
        return <Comp {...otherProps} style={style}>
            <React.Fragment>
