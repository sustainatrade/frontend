    ...
    renderFilters = () => {
        return <PostFeedContext.Consumer>
            {({ setFiltersFn, filters }) => {
                ...
                return <CategoryContext.Consumer>
                    {({ categories: catMap, icons }) => {
                        return <List style={{ padding: 0 }}>
                            <List.Item key='flt-section'>
                                ...
                                <Segment>
                                    <Button.Group fluid>
                                        <Button color={FLTR_BUYING ? 'orange' : FLTR_DEFAULT}
                                            onClick={() => setFiltersFn({ section: (!FLTR_BUYING ? 'buy' : undefined) })}>
                                            Buy
                                </Button>
                                        <Button.Or />
                                        <Button color={FLTR_SELLING ? 'green' : FLTR_DEFAULT}
                                            onClick={() => setFiltersFn({ section: (!FLTR_SELLING ? 'sell' : undefined) })}>
                                            Sell
                                </Button>
                                    </Button.Group>
                                </Segment>
                            ...
                            <List.Item>
                                ...
                                <Menu fluid secondary vertical>
                                    <Menu.Item name='All' active={!filters.category}
                                        onClick={() => {
                                            setFiltersFn({ category: undefined })
                                        }} >All<Icon name="grid layout"/></Menu.Item>
                                    {catMap && Object.keys(catMap)
                                        .map(catKey => <Menu.Item
                                            key={catKey}
                                            name={catMap[catKey]}
                                        ...
                                        >{catMap[catKey]}<Icon name={icons[catKey]}/></Menu.Item>)
                                    }
                                </Menu>
                            </List.Item>
                        </List>
            ...
            <div>
                ...
                >
                    <Menu.Item>
                        ...
                        />
                        <Accordion.Content active={activeIndex === 0} content={<this.renderAccountOpts />} />
                    </Menu.Item>
                    <Menu.Item>
                        <Accordion.Title
    ...
    render() {
        ...
        return (
            <ResponsiveContext.Consumer>
                {({ isMobile }) => <UserContext.Consumer>
                    {({ user, loading }) => {
                        const list = []
                        if (loading) return <div />
                        if(isMobile){
                            list.push(<Menu stackable key='menu-key'>
                        ...
                        list.push(<List.Item key='flters'>{this.renderFilters()}</List.Item>);
