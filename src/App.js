...
export const Menus = ({mobile}) => {
    ...
    return <React.Fragment>
        ...
        <CenterItem href={staticDomain+'/send-feedback'} >Send Feedback</CenterItem>
        <Dropdown item 
            icon='ellipsis horizontal'
            style={mobile?{paddingLeft:'46%'}:undefined}>
          <Dropdown.Menu>
            <CenterItem as={Dropdown} href={staticDomain+'/privacy-policy'} >Privacy</CenterItem>
            <CenterItem as={Dropdown} href={staticDomain+'/terms'} >Terms</CenterItem>
            <CenterItem as={Dropdown} href={staticDomain+'/faq'} >FAQ</CenterItem>
          </Dropdown.Menu>
        </Dropdown>
    </React.Fragment>}
                    ...
                    {({ isMobile }) => {
                        ...
                        return <React.Fragment>
                            <Menu inverted style={{
                                ...
                                {isMobile && <Menu.Item onClick={() => this.setState({ showMobileSidebar: !showMobileSidebar })}>
                                    <Icon size='large' name={!showMobileSidebar ? 'content' : 'x'} />
                                </Menu.Item>}
                            ...
                            <Container fluid>
                                <FacebookProvider appId="512081562521251">
                                    <EcoContent showSidebar={isMobile ? showMobileSidebar : true} />
                                </FacebookProvider>
                            </Container>
                            { isMobile && <Modal visible={showMobileSidebar} 
                                zIndex={900} 
