...
export const Menus = ({ mobile }) => {
  ...
  return (
    <React.Fragment>
      ...
      <Dropdown item icon="ellipsis horizontal" style={mobile ? { paddingLeft: '46%' } : undefined}>
        <Dropdown.Menu>
...
const MainHeader = ({ reponsiveContext: { isMobile }, onSetShowMobileSidebar, showMobileSidebar }) => {
  const headerStyle = {};
  if (!isMobile) {
    headerStyle.zIndex = 999;
  }
  return (
    <Menu className="top-header" style={headerStyle}>
      {!isMobile && (
        <React.Fragment>
          <Menus />
        </React.Fragment>
      ...
      {isMobile && (
        <Menu.Item active={showMobileSidebar} onClick={() => onSetShowMobileSidebar(!showMobileSidebar)}>
          {/* <Icon
                        size="large"
