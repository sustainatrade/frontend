...
class Provider extends React.Component {
    ...
    updateStatus = (name, index, props) => {
        ...
        this.setState({ status: Object.assign(status, { [name]: oldTheStatus }) });
    }
    ...
    state = {
        ...
        isUploading: (name) => {
            ...
            for (const fileObj of theStatus) {
                ...
                if(fileObj.progress === 0 ||(fileObj.progress && fileObj.progress<100))
                    return true;
            }
            return false;
        },
            ...
            for (let i = 0; i < files.length; i++) {
                try {
                    ...
                    if (data.status === 'SUCCESS') {
                        ...
                        let method;
                        if (finished){
                            cfg.btn = (<Button basic size='tiny' color='green' disabled content='Complete' />)
                            method = 'success'
                        }
                        else{
                        ...
                        this.updateStatus(name,i,{progress:100,data:data.data[0]})
                    }
                } catch (err) {
                    console.log('err')//TRACE
                    console.log(err)//TRACE
                    this.updateStatus(name,i,{error:err})
                }
            }
